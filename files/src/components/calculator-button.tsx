	...
	return (
		<button
			...
			onClick={(e) => {
				...
				if (!isIcon) {
					...
					//
						...
						const getLastNumber = (equation: string) => {
							const match = equation.match(/(\d+(\.\d+)?)/g);
						...
						const n = getLastNumber(equation);
						console.log(n);
			...
			{isIcon ? (
				...
				<>
					<sup className='font-semibold text-lg'>{label.split('/')[0]}</sup>
					<span className='font-semibold text-lg'>/</span>
					<sub className='font-semibold text-lg'>{label.split('/')[1]}</sub>
				</>
			) : (
				label
			)}
